...
PHOTU is a photography brand website — a dark-themed marketing/e-commerce site with a hero section, product cards, pricing tiers, and services. Built with TanStack Start and deployed on Netlify.

## Key Directories

```
src/routes/         # File-based routing (TanStack Router)
  __root.tsx        # HTML shell, global head tags, imports styles.css
  index.tsx         # Entire homepage as one React component
src/styles.css      # CSS custom properties, global typography, utility classes
...
| `--accent-pink` | `#e91e8c` | Primary brand color |
| `--accent-orange` | `#ff6b35` | Secondary accent |
| `--accent-cyan` | `#00d4ff` | Info/service accent |

Logo letters use individual colors: P=cyan, H=green, O=yellow, T=red, U=pink.

## Non-Obvious Decisions

- Inline styles are used in `index.tsx` for per-element theming — avoids Tailwind specificity issues with dynamic accent colors.
- Hero camera is a pure SVG illustration (no external image dependency).
- Pricing cards use JS `onMouseEnter`/`onMouseLeave` because hover background must match a dynamic per-card accent color.
- All product/pricing data is currently hardcoded. To add persistence, use `@netlify/database` with Drizzle ORM.
